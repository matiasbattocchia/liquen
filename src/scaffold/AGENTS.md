...
- `config.jsonc` declares everything: the org's identity, the agent roster, the
  connectors. Only the setup doors write it — `mu init` made it, `mu agent <name>` adds
  an agent to the roster (`--name`, `--email`, `--phone` declare who it stands for),
  `mu connect` declares the connection a grant earns. Git is its history, and boot
  compiles it.
