 ...
 * and the door is convention, like the rest of the exec plane. Discovery is the generated
 * stub (`agents/<name>/mu.ts`), written here each boot beside the socket: a script imports
 * `./mu.ts` and the stub binds by its own location — no MU_DOOR variable, no path of ours
 * in user space.
 *
...
/** Serve one door per agent under `dir` and write each folder's client stub. */
export async function installDoors(dir: string, agents: DoorAgent[]): Promise<Doors> {
  const script = new URL("./script.ts", import.meta.url).href;
  ...
  for (const agent of agents) {
    ...
    await Deno.writeTextFile(`${home}/mu.ts`, stub(script));
...
/** The client stub the agent imports (`./mu.ts`): binds `script.ts` to its own folder, so
 *  the socket needs no discovery and no environment. Rewritten every boot — generated. */
function stub(script: string): string {
  return `// generated by mu at start — do not edit (rewritten every boot).
// The door client (§9): import { send, search } from "./mu.ts" in a script beside it.
//   send({to, text})  → queues the ask as one of YOUR gated tool calls; you are woken
//                       with the outcome and narrate it. Returns { id, status: "queued" }
//                       — never the outcome: a script fires and forgets.
//   search({...})     → queues the same way — the hits land in the log for you, next turn.
import { bind } from "${script}";
const mu = bind(new URL(".", import.meta.url).pathname);
export const send = mu.send, search = mu.search;
`;
}
