...
import type { Store, Substrate } from "./mod.ts";

export function lockSuite(s: Substrate): void {
  ...
  /** The lock lives in the store, so a second handle on it is another process's view of
   *  the same lease (§2). */
  async function withLogs(
    fn: (a: Log, b: Log, store: Store, t: ReturnType<typeof clock>) => Promise<void>,
  ): Promise<void> {
    const store = await s.fresh();
    const t = clock();
    const a = await store.open({ now: t.now });
    const b = await store.open({ now: t.now });
    try {
      await fn(a, b, store, t);
    } finally {
      await a.close();
      await b.close();
      await store.drop();
  ...
  async function crashedHolder(store: Store, name: string) {
    const dead = await store.open();
  ...
  Deno.test("a crashed holder stops beating, and its lease is stolen after the TTL", async () => {
    await withLogs(async (one, _two, store, t) => {
      await crashedHolder(store, "turn-a1");
  ...
  Deno.test("a zombie cannot write the log: its turn's events are refused, the successor keeps the lease", async () => {
    await withLogs(async (one, _two, store, t) => {
      ...
      const stale = await crashedHolder(store, "turn-a1");
      t.advance(LOCK_TTL_MS + 1);
  ...
  Deno.test("two stealers race for one stale lease: exactly one wins", async () => {
    await withLogs(async (one, two, store, t) => {
      await crashedHolder(store, "turn-a1");
      t.advance(LOCK_TTL_MS + 1);
