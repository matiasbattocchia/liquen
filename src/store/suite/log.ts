...
export function logSuite(s: Substrate): void {
  ...
  const withLog = (fn: (log: Log) => Promise<void>) => withStore(s, fn);
  ...
  Deno.test("read `conversations` scopes to a set — the readable filter at source (§6)", async () => {
    await withLog(async (log) => {
      ...
      assertEquals((await log.read({ conversations: ["c1", "c3"] })).map((e) => e.id), [
        "01",
        "03",
      ]);
    });
  ...
  Deno.test("two processes publishing at once: every row commits, none is lost to contention", async () => {
    const store = await s.fresh();
    try {
      // two independent Log handles = two connections contending for the same store
      const w1 = await store.open();
      const w2 = await store.open();
  ...
  Deno.test("subscribe reads the store, not writer memory (cross-process path)", async () => {
    const store = await s.fresh();
    try {
      const writer = await store.open();
      const consumer = await store.open(); // a *different* handle, as another process would
