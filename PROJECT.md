...
The address-book leg landed on `main` the same afternoon in the JSON shape (`messages` /
`contacts`, the entry above) and was folded into the page at the merge: a book's entries
stand FIRST — who they are, before what was said — as `<contact name address/>` lines under
the `<conn>` of the account holding them (the same element its traffic wears; `contact` is
already the window's word for a saved sender), and a book that could not be asked is one
harness line, `— whatsapp 5491… "Sole": address book not reached —`, the account named as
the `connections:` line names it. `Contacts` is what `booked` answers with; `SearchResult`
