...
they are saved in — so both are asked and the answer has a leg for each: the rows under
`messages`, the entries under `contacts`, each tagged with the account holding it. That is
...
asks whichever accounts have `lookup`. A book that cannot be reached is named in
`contacts.unreached` rather than failing the search, because the log is the answer being
asked for.
